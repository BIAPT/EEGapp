...
% function rest_CallBrainNetViewer(BrainVolume,NMin,PMin,ClusterSize,ConnectivityCriterion,SurfFileName,viewtype,ColorMap,NMax,PMax,BrainHeader)
...
%     viewtype     - The type of view. Could be 'FullView' (8 views) or 'MediumView' (4 views)
...
if ischar(BrainVolume)
    ...
    BrainHeader = BrainNet_spm_vol(BrainVolume); %%% Edited by MingruiXia 111026, using spm to read NIFTI files.
    BrainVolume = BrainNet_spm_read_vols(BrainHeader);
...
if exist('ClusterSize','var') && ClusterSize > 0
