...
for subject = 1
   switch subject
    ...
    for state = 1:10
        ...
        PLIfile = load(['F:\McDonnell Foundation study\University of Michigan\Anesthesia\' sname '\Resting state analysis\' sname statename '_PLI.mat']);
        bpPLI = PLIfile.PLI{4};
        
        dPLIfile = load(['F:\McDonnell Foundation study\University of Michigan\Anesthesia\' sname '\dPLI analysis\' sname statename '_dPLI.mat']);
        dPLI = dPLIfile.dPLI{4};    % 4 gives me alpha
        ...
        for i = 1:length(EEG_bpPLI)
          for j = 1:length(EEG_bpPLI)
              if (EEG_bpPLI(i,j) > thresh)
                  dPLInetwork(i,j) = dPLI(i,j);
              else
                  dPLInetwork(i,j) = -2;        % No connection
              end
          end
        end

        dlmwrite(['F:\McDonnell Foundation study\University of Michigan\Anesthesia\' sname '\Resting state analysis\' sname statename '_dPLI network.txt'], dPLInetwork);
