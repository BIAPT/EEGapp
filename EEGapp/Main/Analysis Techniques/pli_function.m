...
try
    ...
    for sub=1:plot_number
        ...
        settings = evalin('base','settings');
        if(settings.options.custom_plot_pli == 1)
           [custom_x_labels,custom_y_labels,custom_x_ticks,custom_y_ticks] = load_plot_data('custom_plot_pli.fig');
            set(gca,'YTicklabel',custom_y_labels)
            set(gca,'XTickLabel',custom_x_labels)%[0:step:max_time])
            set(gca,'YTick',custom_y_ticks)
            set(gca,'XTick',custom_x_ticks)%[0:step:max_time])            
        end
        colorbar;
